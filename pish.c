...
#include "pish.h"
//used chatGPT to debug errors found in these methods
/*
...
void parse_command(char *command, struct pish_arg *arg)
{
    ...
    arg->argc = 0;
    memset(arg->argv, 0, sizeof(arg->argv));  // Clear out argv
    ...
    arg->argv[arg->argc] = NULL;  // Null-terminate the array
}
...
void run(struct pish_arg *arg)
{
    ...
    if (strcmp(arg->argv[0], "exit") == 0) {
        ...
        // Non-built-in command: fork and execvp
        pid_t pid = fork();
        if (pid == -1) {
            perror("fork");
        } else if (pid == 0) {
            execvp(arg->argv[0], arg->argv);
            perror("pish");  // Only reached if execvp fails
            exit(EXIT_FAILURE);
        } else {
